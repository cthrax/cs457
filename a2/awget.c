...
//	rep_p.version = htons(1);
	...
	if(argc !=5)
	{
		printf("Enter Correct number of arguments in any order \n  %s -u <URL> -c <chain file> \n", argv[0]) ;
		exit(1);
	}	
...
//		printf("Port : %s\n", tok);
	...
	else
	{
		...
		for(i=0; i<hop; i++)
		{
			printf("<%s(%d), %d>\n", cip[i].ch_ip, itp[i].ip_addr, itp[i].port_no);
	...
	// Conenct to SS
	if((connect(cli_sd, (struct sockaddr *)&server_sck_addr, mem_offset)) != 0 )
	{
		perror("Connect");
		exit(1);
	}
